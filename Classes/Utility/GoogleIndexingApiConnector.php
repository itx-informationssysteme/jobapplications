...
class GoogleIndexingApiConnector
{
     ...
     * @param                    $uid
     * @param bool               $delete
     * @param array|Posting|null $specificPosting
     *
     * @return bool|null
     * @throws \JsonException
     */
    public function updateGoogleIndex($uid, bool $delete, $specificPosting = null): ?bool
    {
        ...
        $contentElements = $this->ttContentRepository->findByCType('jobapplications_frontend');
    ...
    private function sendFlashMessage(string $msg, string $header = '', bool $error = false): void
    {
        ...
        if ($error) {
            $type = ContextualFeedbackSeverity::WARNING;
        }
    ...
    public function makeRequest(string $url, bool $deleteInsteadOfUpdate): ?bool
    {
        ...
        if (is_array($sessionData) && ($sessionData['indexingAccessToken'] ?? null)) {
    ...
    private function makeAuthReq(): string
    {
        ...
        try {
            ...
            // Session caching is unavailable outside a real backend HTTP request
        }
